...
from .reminder_utils import schedule_task_reminder, cancel_task_reminder, reschedule_task_reminder
import json
...
def create_task(request):
    try:
        ...
        if task.due_date and task.due_time:
            task_id = schedule_task_reminder(task)
            if task_id:
                task.reminder_task_id = task_id
...
def toggle_task(request, task_id):
    try:
        ...
        if task.completed and task.reminder_task_id:
            cancel_task_reminder(task)
            task.reminder_task_id = None
        elif not task.completed and task.due_date and task.due_time:
            reschedule_task_reminder(task)
...
def update_task(request, task_id):
    try:
        ...
        if (task.due_date != old_due_date or task.due_time != old_due_time):
            if task.due_date and task.due_time and not task.completed:
                reschedule_task_reminder(task)
            elif task.reminder_task_id:
                cancel_task_reminder(task)
                task.reminder_task_id = None
                task.save(update_fields=['reminder_task_id'])
